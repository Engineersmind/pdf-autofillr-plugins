...
def investor_pipeline() -> None:
    ...
    print(f"  2. Mapped {len(mapping['mapped_fields'])}/{len(extraction['fields'])} fields  "
    ...
    result = validator.validate("email_address", email_value)
...
def invoice_pipeline() -> None:
    ...
    if extractor:
        ...
        if mapper:
            ...
            for src, tgt in [(i["source"], i["target"]) for i in mapping["mapping_info"]]:
                print(f"       {src:<20} → {tgt}")
