...
def bench_plugin_load() -> dict:
    ...
    return {"task": "load_plugin", "plugin": "email-validator", "duration_ms": round(duration_ms, 2)}
...
def bench_validate_throughput(n: int = 1000) -> dict:
    ...
    manager.shutdown()
    print(f"  ✅  validate {n} emails             {per_item_ms:.3f}ms/item  ({items_per_sec:,} items/sec)  {success_rate:.0%} valid")
    return {
...
def bench_ml_mapper_throughput(n: int = 500) -> dict:
    ...
    start = time.perf_counter()
    for _ in range(n):
        mapper.map_fields(fields_batch)
    duration = time.perf_counter() - start

    per_call_ms = duration / n * 1000
    manager.shutdown()
    print(f"  ✅  ml-mapper {n} calls             {per_call_ms:.3f}ms/call  ({int(1000/per_call_ms):,} calls/sec)")
    return {
...
'''
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        print(f"  ✅  discovery from dir           {duration_ms:.2f}ms  ({n_discovered} plugin(s) found)")
        return {
            "task": "discovery_time",
            "duration_ms": round(duration_ms, 2),
